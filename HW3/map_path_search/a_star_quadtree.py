...
def a_star_quadtree(start_m, goal_m, qtm, movement='8n', occupancy_cost_factor=3):
    ...
    total_est_cost_ini = quadtreemap.Point.disOf2Points(start.getCenter(),goal.getCenter())
    print("total est cost = ",total_est_cost_ini)
    candidates.put((total_est_cost_ini, 0, None, start))   # store (est-total-distance-to-goal,distance-to-curr-node, previous-tile, current-tile)
    while candidates:
        total_est_cost, dis, prev_node, curr_node = candidates.get()
